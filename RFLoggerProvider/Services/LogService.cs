...
public class LogService(
    ...
    : CommonEntityService<Log>(logRepository, serviceProvider),
    ILogService
{
    ...
    public async Task<Log> AddAsync(LLevel level, LAction action, string message, object? data = null, bool? dataRequest = null)
    {
        ...
        var levelId = await logLevelService.GetSingleIdOrCreateByNameAsync(level.ToString());
        var actionId = await logActionService.GetSingleIdOrCreateByNameAsync(action.ToString());
